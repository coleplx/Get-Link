...
var getInfo = function(callback) {
	...
	request(options, function (error, response, body) {
        ...
        result.uid = S(body).between('edit_link_', '"').s
        result.token = S(body).between('authenticity_token" value="', '"').s
...
var skip = function(callback) {
    var options = {
		uri: 'http://admy.link/links/' + result.uid,
		method: 'POST',
        form: {
            utf8: '✓',
            _method: 'patch',
            authenticity_token: result.token
        },
		headers: {
    		"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.60 Safari/537.36"
		},
	...
	request(options, function (error, response, body) {
        ...
        result.direct_link = response.headers['location']

        getTitle(result.direct_link, function(err, title) {
            if (err)
                title = '7.LY'
