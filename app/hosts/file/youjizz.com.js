...
var re = /https?:\/\/(www\.)?youjizz\.com\/videos\/.+/
var result = {}
...
var getFileInfo = function(callback) {
	...
	request(options, function (error, response, body) {
        ...
        if (_.includes(body, 'This video does not exist or has been removed.'))
            return callback(new Error('Video không hợp lệ hoặc đã bị xóa'))

		result.file = {
			name: S(body).between('<title>', '</title>').s,
			type: 'mp4'
		}
		result.direct_link = S(body).between('newLink.setAttribute(\'href\',\'', '\'').s
