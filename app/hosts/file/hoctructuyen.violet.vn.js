...
var re = /http:\/\/hoctructuyen.violet.vn\/present\/show\/entry_id\/(\d+)/
var result = {}
...
var login = function(username, password, callback) {
	var options = {
		uri: 'http://hoctructuyen.violet.vn/user/login',
		method: 'POST',
		form: {
    		"_": "",
    		"username": username,
...
var getFileInfo = function(callback) {
	...
	request(options, function (error, response, body) {
        ...
        result.download_id = S(body).between('Modalbox.show(\'http://hoctructuyen.violet.vn/present/predownload/pr_id/', '/').s
...
var download = function(callback) {
	var options = {
		uri: 'http://hoctructuyen.violet.vn/present/download/pr_id/' + result.download_id,
		method: 'HEAD',
		headers: {
    		"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.60 Safari/537.36"
		},
