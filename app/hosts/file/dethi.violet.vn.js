...
var re = /http:\/\/dethi.violet.vn\/present\/show\/entry_id\/(\d+)/
var result = {}
...
var login = function(username, password, callback) {
	var options = {
		uri: 'http://dethi.violet.vn/user/login',
		method: 'POST',
		form: {
    		"_": "",
    		"username": username,
...
var getFileInfo = function(callback) {
	...
	request(options, function (error, response, body) {
        ...
        result.download_id = S(body).between('Modalbox.show(\'http://dethi.violet.vn/present/predownload/pr_id/', '/modalbox').s
...
var download = function(callback) {
	var options = {
		uri: 'http://dethi.violet.vn/present/download/pr_id/' + result.download_id,
		method: 'HEAD',
		headers: {
    		"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.60 Safari/537.36"
		},
	...
	request(options, function (error, response, body) {
        ...
        if (response.statusCode !== 301 && response.statusCode != 302)
            return callback(new Error('Lỗi khi direct link'))
