...
var re = /http:\/\/lophoc.violet.vn\/document\/show\/entry_id\/(\d+)/
var result = {}
...
var login = function(username, password, callback) {
	var options = {
		uri: 'http://lophoc.violet.vn/user/login',
		method: 'POST',
		form: {
    		"_": "",
    		"username": username,
...
var getFileInfo = function(callback) {
	...
	request(options, function (error, response, body) {
		...
		result.file = {
			name: S(body).between('<div class="doc document">', '<label>').strip('<h2>').trim().s
		}
        result.download_id = S(body).between('/document/download/doc_id/', '"').s
...
var download = function(callback) {
	var options = {
		uri: 'http://lophoc.violet.vn/document/download/doc_id/' + result.download_id,
		method: 'HEAD',
		headers: {
    		"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.60 Safari/537.36"
		},
